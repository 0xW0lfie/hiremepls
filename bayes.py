...
SQ1_CORNERS = (130, 265, 180, 315) # (UL-X, UL-Y, LR-X, LR-Y)
SQ2_CORNERS = (80, 255, 130, 305) # (UL-X, UL-Y, LR-X, LR-Y)
SQ3_CORNERS = (105, 205, 155, 255) # (UL-X, UL-Y, LR-X, LR-Y)
...
class Search():
    ...
    def __init__(self, name):
        ...
        self.area_actual = 0
        self.sailor_actual = [0, 0] # As "local" coords within search area_actual
    ...
    def draw_map(self, last_known):
        ...
        cv.putText(self.img, '+ = Last Knwon Position', (274, 355), cv.FONT_HERSHEY_PLAIN, 1, (0, 0, 255))
    ...
    def sailor_final_location(self, num_search_areas):
        """Return the actual x,y location of missing sailor."""
        self.sailor_actual[0] = np.random.choice(self.sa1.shape[1])
        self.sailor_actual[1] = np.random.choice(self.sa1.shape[0])

        area = int(random.triangular(1, num_search_areas + 1))

        # Location is actually static to make prinicple simpler.
        # In real life, the sailor would most likely drift into
        # area 3 after each search
        if area == 1:
    ...
    def conduct_search(self, area_num, area_array, effectiveness_prob):
        ...
        coords = list(itertools.product(local_x_range, local_y_range)) # itertools creates a  list of all possible combinations of the two ranges.
        random.shuffle(coords) # Shuffle the itertools list to prevent starting from the same area
...
def main():
    ...
    while True:
        ...
        app.revise_target_probs() # Use Bayes' rule to update target probability
        ...
        if results_1 == 'Not Found' and results_2 == 'Not Found':
            print("\New Target Probabilities (P) for Search {}:".format(search_num + 1))
            print("P1 = {:.3f}, P2 = {:.3f}, P3 = {:.3f}".format(app.p1, app.p2, app.p3))
        else:
